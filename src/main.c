...
=======
int min3(int a, int b, int c){// funcion auxiliar para livenstein
>>>>>>> dde4c4a2e713fe8654a85c8ccd34f7ffa3707253
  int m=a;
  if(b<m){m=b;}
  if(c<m){m=c;}
  return m;
...
=======
if (s1[i - 1] ==
    s2[j - 1]) { // determinamos el coste dependiendo si la substitucion por el
                 // numero de la diagonal es porque las letras son las mismas o
                 // porque es una sustitucion
...
matrix[i][j] =
    min3(matrix[i - 1][j] + 1, matrix[i][j - 1] + 1,
         matrix[i - 1][j - 1] + cost); // buscamos el valor mas pequeños de los
