...
place* get_map_places(char* map_name){
  ...
  while (fgets(line, sizeof(line), f)){
    ...
    if (new_p == NULL) return head; //si la memoria falla, retornem el que ja tenim
    line[strcspn(line, "\n")] = '\0'; //eliminem el salt de linea final 

    char* token = strtok(line, ";");  //busquem el primer tros, que sera el nom del lloc
    if (token == NULL) continue;  //si la linea esta buida, saltem al la seguent
    ...
    if (head == NULL){  //si la llista esta buida el nou lloc es el primer i tmable l'ultim
      head = new_p;
      tail = new_p;
    } else{ // si ja hi tenim algun altre element, l'antiga cua ara apunta al nou element, i el nou element pasa a ser la nova cua
      tail->next = new_p;
      tail = new_p;
    }
  }
  ...
  // Comprovem si ha triat "coordinate" o "place" 
    if (strcmp(input_type, "coordinate") == 0) {
      ...
      while (curr_p != NULL){ //mirem cada lloc de la llista fins al final
        if (strcmp(curr_p->name, search_place) == 0){ //si el nom es exactament igual al buscat
      ...
      if (!found_p){  //si la cerca exacta ha fallat, foun_p = 0
        ...
        while (curr_p != NULL && sug_count < 5){  //mirem tota la llista i parem quan en tinguem 5 opcions
        ...
        if (sug_count == 0){  //si finalment no trobem cap lloc semblant al que busca l'usuari diem  que no hem trobat res
