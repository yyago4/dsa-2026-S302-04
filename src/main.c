...
place *get_map_places(char *map_name)
{
  ...
  sprintf(filename, "%s_places.txt", map_name); // montem el nom real

  FILE *f = fopen(filename, "r"); // obrim el fitxer en mode lectura
  if (f == NULL)
  { // si no hem pogut obrir el fitxer, avisem i retornem Null
    printf("Error opening places fil.\n");
    return NULL;
  }

  place *head = NULL;     // punter el primer element de la llista
  place *tail = NULL;     // punter a l'ultim element de la llista
  char line[MAX_STR * 2]; // espai per poder guardar temporalment cada linea que hem llegit

  while (fgets(line, sizeof(line), f))
  {
    place *new_p = (place *)malloc(sizeof(place)); // reservem memoria ram per a un node nou
    if (new_p == NULL)
      return head;                    // si la memoria falla, retornem el que ja tenim
    line[strcspn(line, "\n")] = '\0'; // eliminem el salt de linea final

    char *token = strtok(line, ";"); // busquem el primer tros, que sera el nom del lloc
    if (token == NULL)
    ...
    if (token == NULL)
      continue;               // si la linea esta buida, saltem al la seguent
    new_p->lat = atof(token); // convertim el text de latitud a numero real doble, de ascii a float

    token = strtok(NULL, ";"); // busquem el primer tros, que sera la longitud
    if (token == NULL)
      continue;               // si la linea esta buida, saltem al la seguent
    new_p->lon = atof(token); // convertim el text de longitud a numero real doble, de ascii a float

    new_p->next = NULL; // definim que el nou lloc, inicilment no te ningu al darrere

    if (head == NULL)
    { // si la llista esta buida el nou lloc es el primer i tmable l'ultim
      head = new_p;
      tail = new_p;
    }
    else
    { // si ja hi tenim algun altre element, l'antiga cua ara apunta al nou element, i el nou element pasa a ser la nova cua
      tail->next = new_p;
      tail = new_p;
    }
  }
...
{ // amb aquesta funció ajustarem el text segons majusculas, minusucules i abreviatures
  for (int i = 0; str[i]; i++)
  {                           // anem lletra per lletra del text
  ...
  { // en el cas de que hi hagi l'abreviature c., la convertim en carrer
    if (str[2] == ' ')
    { // si és c. amb un espai "c. " saltem 3 espais
      offset = 3;
    }
    else
    {
  ...
  else if (strncmp(str, "av.", 3) == 0)
  { // en el cas de que hi hagi l'abreviature av., la convertim en avinguda
    if (str[3] == ' ')
    { // si és av. amb un espai "av. " saltem 4 espais
      offset = 4;
    }
    else
    { // si no te espai "av." saltem 3 espais
      offset = 3;
    }
  ...
  else if (strncmp(str, "ptge.", 5) == 0)
  { // en el cas de que hi hagi l'abreviature ptge., la convertim en passatge
    if (str[5] == ' ')
    { // si és ptge. amb un espai "pte. " saltem 6 espais
      offset = 6;
    }
    else
    { // si no te espai "av." saltem 5 espais
      offset = 5;
    }
...
int count_matches(const char *s1, const char *s2)
{ // implementamos una funcion que compara los caracteres de una str con la otra en las miismas posiciones
  int matches = 0;
...
int main()
{
  ...
  {
    char search_place[MAX_STR]; // creem espai per a guardar el nom que escrigui l'usuari

    printf("Enter a place name: ");
    scanf(" %[^\n]", search_place); // lleguim tota la frese que escrigui l'usuari incloent espais
    ...
    while (curr_p != NULL)
    { // mirem cada lloc de la llista fins al final
      if (strcmp(curr_p->name, search_place) == 0)
      {                                                          // si el nom es exactament igual al buscat
        printf("Found at (%f, %f)\n", curr_p->lat, curr_p->lon); // mostrem les coordenades
    ...
    if (!found_p)
    {                                             // si la cerca exacta ha fallat, foun_p = 0
      printf("Place not know. Did you mean: \n"); // avisem que ara buscarem similars
      curr_p = place_list;                        // tonem a començar des de l'inici de la llista de llocs
      int sug_count = 0;                          // contador per nomes mostrar certa quantitat de sugeriments

      while (curr_p != NULL && sug_count < 5)
      { // mirem tota la llista i parem quan en tinguem 5 opcions
        if (strstr(curr_p->name, search_place) != NULL)
        {
          printf("- %s (at %f, %f)\n", curr_p->name, curr_p->lat, curr_p->lon); // ensenyem la opcio
          sug_count++;                                                          // sumem un sugeriment a la llista
        }
        curr_p = curr_p->next; // pasem al seguent lloc per seguir comprovant
      }
      if (sug_count == 0)
      { // si finalment no trobem cap lloc semblant al que busca l'usuari diem  que no hem trobat res
  ...
  else if (strcmp(input_type, "address") == 0)
  {
    ...
    while (current != NULL)
    {                                    // mirem casa per casa fins arribar al final de la llista
      ...
      if (strcmp(current_street_norm, search_norm) == 0)
      { // comprovem si el carrer normalitzat .
        street_exists = 1;
        if (current->num == address_number)
        {                                                                      // si el numero tambe coincideix
    ...
    if (!found)
    { // si hem mirat totes i no hem trobat res
      if (street_exists)
      { // caso en que la calle es correcta pero el numero no
        ...
        while (current != NULL)
        {
          ...
          if (strcmp(csn, search_norm) == 0)
          {                                // miramos si scn(calle actual normalizada) y serach_norm(calle buscada normalizada) son las mismas
      ...
      { // caso que la calle no se sepa
        ...
        while (current != NULL)
        {
          ...
          if (matches >= strlen(search_norm) / 2 && matches > 2)
          {                      // condiciones para entrar en la lista: parecidos almenos la mitad de la longitud de la str y >2
            int already_in = -1; // auxiliar para ver si la calle ya esta en la lista
            for (int i = 0; i < sug_count; i++)
            { // la primera iteracion se lo salta ya que es la primera calle y si o si no esta en la lista
            ...
            if (already_in == -1)
            { // si la calle es nueva probamos de meterla en el top
              for (int i = 0; i < 5; i++)
              {
                if (matches > max_matches[i])
                { // si la calle actual es mejor que la del top en posicion i
                  for (int j = 4; j > i; j--)
                  { // movemos todos los de abajo una posicion abajo
